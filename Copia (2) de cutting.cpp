...
//		if (m_iter_actual%200==0) m_tam_tabu=get_random(0.25*m_Q,1*m_Q);
		if (m_iter_sin_mejorar%250==0 && m_iter_actual>0)
		{
...
//		if (m_lista_tabu.size()>0 && m_max_valor==(-250000) ) 
		if ((m_lista_tabu_perdidas.size()>0 && valempate==(-250000) && valempate2!=(-250000) ) ||m_max_valor==(-250000)) 
		{
...
//			m_lista_tabu_aumen.pop_front();
		...
		if (m_max_valor==(-250000) && m_lista_tabu_new.size()==0)
		{
...
//	printf("Iter %d Best %d\n",m_iter_actual,m_solbest);
	}while((m_iter_sin_mejorar<10000) && (m_iter_actual<m_max_iteraciones) && (m_solbest!=m_Cota) && (m_max_valor!=(-250000)));
...
//	DibujarSolucion(movimiento.GetListaBloques(),movimiento.GetListaPerdidas(),888,888);
	...
	movimiento.SetValor(Meto-Quito);
	int temporal=EvaluarMovimiento(movimiento);
	if (temporal<m_max_valor  && (m_sol_actual+movimiento.GetValor()!=m_Cota))
	{
		movimiento.SetAlgoNuevo(false);
		return movimiento;
	}
...
//	int bien=0;
	...
	CRect Union(0,0,0,0);
	int max=m_lista_tabu_perdidas.size()+1;
	int area1=Contar(M1.GetListaPerdidas());
