...
//	printf("Iter %d Best %d\n",m_iter_actual,m_solbest);
	}while((m_iter_actual<m_max_iteraciones) && (m_solbest!=m_Cota) && (m_max_valor!=(-250000)));
...
//	DibujarSolucion(movimiento.GetListaBloques(),movimiento.GetListaPerdidas(),888,888);
	VerificarSolucion(movimiento.GetListaBloques(),movimiento.GetListaPerdidas(),7);
//	Bloques_Hacia_Las_Esquinas(movimiento.GetListaBloques(),movimiento.GetListaPerdidas());
	Bloques_Hacia_Las_Esquinas(movimiento.GetListaBloques().back(),movimiento.GetListaPerdidas());
//	JuntarPerdida(movimiento.GetListaPerdidas(),'P');
	int kk=JuntarPerdida(movimiento.GetListaPerdidas(),B1Aumen,'P');
...
//		if (m_iter_actual%200<=100)
		int dif=0;
		if (m_iter_sin_mejorar==1000) 
		{
			dif=m_iter_sin_mejorar/100;
		}
//		max_candidatos-=(Rectangulo_menor_estan_perdidas(Movimiento.GetListaPerdidas())/10)*(6-dif);
//		max_candidatos-=(Rectangulo_menor_estan_perdidas(Movimiento.GetListaPerdidas()));
//		else 
		max_candidatos+=Movimiento.GetValor();
		max_candidatos-=((Movimiento.GetListaPerdidas().size()));

		if (m_iter_sin_mejorar%50==0) 
		max_candidatos-=Rectangulo_menor_estan_perdidas(Movimiento.GetListaPerdidas());
//			max_candidatos=-0.1*max_candidatos;
		if (m_iter_sin_mejorar%40==0) 
			max_candidatos=max_candidatos/50;
		...
		{
			int caso=(m_iter_sin_mejorar-400)/100;
			switch (caso)
			{
				case 0:
				{
...
int CUTTING::Valida_Simetria(std::list<RECTAN> &lista)
{
	if (m_Simetria==false)	return true;
	double mitadPH=0,mitadPV=0,pricuaP=0,totalP=0,trescuartosP=0;
	...
	for (itB1=lista.begin();itB1!=lista.end();itB1++)
	{
		if ((*itB1).left<(double)m_L/2)
		{
			...
			{
				mitadPV+=(*itB1).Height()*(*itB1).Width();
			}
			else
			{
				mitadPV+=(*itB1).Height()*(((double)m_L/2)-(*itB1).left);
			}
		}
	}
	for (itB1=lista.begin();itB1!=lista.end();itB1++)
	{
		if ((*itB1).top<(double)m_W/2)
		{
			...
			{
				mitadPH+=(*itB1).Height()*(*itB1).Width();
			}
			else
			{
				mitadPH+=(*itB1).Width()*(((double)m_W/2)-(*itB1).top);
			}
		}
	}
