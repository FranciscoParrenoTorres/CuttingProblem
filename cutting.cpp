...
//Ordenada de la más ancha a la más estrecha
struct orden_strip
{
	bool operator() (const Pieza& p1, const Pieza& p2);
};
...
struct orden_stripAltura
{
	bool operator() (const Pieza& p1, const Pieza& p2);
};
bool orden_stripAltura::operator() (const Pieza& p1, const Pieza& p2){
	if (p1.largo==p2.largo)
			return (p1.ancho)>(p2.ancho);
	else
			return (p1.largo)>(p2.largo);
...
void CUTTING::FijarPiezasInicialmente()
{
	//Si no existe ninguna pos ya esta no
	//fijo ninguna
	int inicio_total=0;
	int area_coloco=0;
	bool pueda_fijar=true;
	do 
	{
		bool finB=false,finS=false;
		int sum1=0,sum2=0;
		if (m_Piezas[0].ancho<=m_W/2 ) 
		{
			m_TrozoFijado=inicio_total;

			return;
		}
		int uvedoble1=m_Piezas[0].ancho;
		std::vector<int> VectorB;
		std::vector<int> VectorS;
		for (register int i=0;i<m_Numpiezas && finB!=true;i++)
		{
			//Conjunto B
			if (m_Piezas[i].ancho==uvedoble1)
			{
				VectorB.push_back(i);
				sum2+=m_Piezas[i].largo*m_Piezas[i].max;

			}
			else finB=true;
			
		}
		for (int i=m_Numpiezas-1;i>=0 && finS!=true;i--)
		{
			//Conjunto S
			if (m_Piezas[i].ancho<=(m_W-uvedoble1))
			{
				VectorS.push_back(i);
				sum1+=m_Piezas[i].largo*m_Piezas[i].max;
			}
			else finS=true;
		}
		//Si se cumple podemos fijarlos
		if (sum1<=sum2 && sum2!=0)
		{
			int inicio_altura=inicio_total;
			std::vector<int> ::iterator it;
			for (it=VectorB.begin();it!=VectorB.end();it++)
			{
				//B(l,t,r,b)
				int kk=(*it);
				int ancho=m_Piezas[(*it)].ancho;
				BLOQUE b1(inicio_altura,0,inicio_altura+(m_Piezas[(*it)].largo*m_Piezas[(*it)].max),m_Piezas[(*it)].ancho);
				inicio_altura+=(m_Piezas[(*it)].largo*m_Piezas[(*it)].max);
				m_lista_bloques_fijos.push_back(b1);
				m_total_superficie-=m_Piezas[(*it)].largo*m_Piezas[(*it)].max*m_Piezas[(*it)].ancho;
				area_coloco+=b1.Width()*b1.Height();
			}
			inicio_altura=inicio_total;
			for (std::vector<int> ::iterator it=VectorS.begin();it!=VectorS.end();it++)
			{
				BLOQUE b1(inicio_altura,m_W-m_Piezas[(*it)].ancho,inicio_altura+(m_Piezas[(*it)].largo*m_Piezas[(*it)].max),m_W);
				inicio_altura+=(m_Piezas[(*it)].largo*m_Piezas[(*it)].max);
				m_lista_bloques_fijos.push_back(b1);
				m_total_superficie-=m_Piezas[(*it)].largo*m_Piezas[(*it)].max*m_Piezas[(*it)].ancho;
				area_coloco+=b1.Width()*b1.Height();
			}
			//Borro de delante a atrás para que se mantenga
			std::vector<int> ::reverse_iterator itr;
			for (std::vector<int> ::reverse_iterator itr=VectorB.rbegin();itr!=VectorB.rend();itr++)
			{
				auto elemento_borrar = m_Piezas.begin() + (*itr);
				m_Piezas.erase(elemento_borrar);
//				m_Piezas.erase(&m_Piezas[(*itr)]);
//				Pieza p = m_Piezas[(*itr)];
//				m_Piezas.erase(m_Piezas[(*itr)]);
//				m_Piezas.erase(itr);
			...
			{
				int kk=(*it);
				auto elemento_borrar = m_Piezas.begin() + ((*it) - VectorB.size());
				m_Piezas.erase(elemento_borrar);
//				m_Piezas.erase(&m_Piezas[(*it)-VectorB.size()]);
				m_Numpiezas--;
			}

			inicio_total+=sum2;
//			for (int ki=0;ki<m_Numpiezas;ki++)
//			printf("alguna pieza se ha podido fijar");
		}
		else pueda_fijar=false;

	}while(pueda_fijar==true);
	m_TrozoFijado=inicio_total;
	m_PerdidaFijado=m_W*inicio_total-area_coloco;
	//Solución óptima conseguida
//	if (m_Numpiezas==0)
...
void CUTTING::JuntarPiezasIguales()
{
	...
	for (register int i=0;juntar!=true && i<m_Numpiezas-1;i++)
	{
		...
		if (m_Piezas[i].ancho==m_Piezas[i+1].ancho && m_Piezas[i].largo==m_Piezas[i+1].largo)
		{
			...
			juntar=true;
			m_Numpiezas--;
				auto elemento_borrar = m_Piezas.begin() + (i + 1);
			m_Piezas.erase(elemento_borrar);
...
/*
			...
			if (m_Piezas[i].largo<=Rect.Width() && m_Piezas[i].ancho*j<=Rect.Height())
			{
				...
				//La más ancha con algo de lo largo
				if (tipo==8) val=m_Piezas[i].ancho*j+m_Piezas[i].largo*(0.35-m_FactorAleatorio2);
				int a1=0,a2=0;
...
//					}
				...
				if (tipo==8) val=m_Piezas[i].ancho*j+m_Piezas[i].largo*m_FactorAleatorio2;
...
int CUTTING::Tabu2DStrip(bool dibu,bool sol_grasp,int a)
{
	m_max_iteraciones=a;
...
//	m_solbest=m_ValSolucion;
	...
	m_solbest=solucion;
//	EscribirSolucionStripLatex(m_solbest);
	return m_solbest;
}
...
int CUTTING::Tabu(bool dibu)
{
	...
	struct timeb t1, t_initial;
	ftime(&t_initial);
	double time_v1 = 0;
	if (m_empieza_sol_grasp == false)
/*	{
		if (GetPuedenRotarsePiezasGrasp()==false)
		HeuristicoGreedyOrdenStrip(false);
		else
			HeuristicoGreedyOrdenStripRotacion(false);
	}*/
	HeuristicoGreedyOrden(false);
...
//			EscribirSolucionLatex(m_lista_bloques);
		...
		ftime(&t1);
		time_v1 = ((double)((t1.time - t_initial.time) * 1000 + t1.millitm - t_initial.millitm)) / 1000;


//	printf("Iter %d Best %d\n",m_iter_actual,m_solbest);
	}while((time_v1<m_total_time) && (m_iter_sin_mejorar<100000) && (m_iter_actual<m_max_iteraciones) && (m_solbest!=m_Cota) && (m_max_valor!=(INT_MIN)));
...
*/	//Para hacer el reactive de 0.2 a 0.9 de 
	...
	if (m_Numpiezas==0) 
	{
		m_solbest=m_TrozoFijado;
		return m_TrozoFijado;
	}
...
/*		if (m_PuedenRotarsePiezas==false)
		{
			int cotaL2=CotaInferiorL2();
			if (cotaL2+m_TrozoFijado>m_Cota)
			{
				printf("L2 %d",cotaL2+m_TrozoFijado);
				m_Cota=cotaL2+m_TrozoFijado;
			}
			int cotaL1=CotaInferiorL1();
			if (cotaL1+m_TrozoFijado>m_Cota)
			{
				printf("L1 %d",cotaL1+m_TrozoFijado);
				m_Cota=cotaL1+m_TrozoFijado;
			}

		}
*/	}
	//Puede darse el caso que tenga un problema en el que todas las piezas hayan 
	//sido fijadas por tanto aqui que hago
	...
	double time_v1 = 0;
//	for (register int iter=0;iter<m_max_iteracionesGrasp;iter++)
	for (m_iter_actual=0;m_iter_actual<m_max_iteracionesGrasp && (time_v1 < m_total_time);m_iter_actual++)
	{	
		ftime(&t1);
		time_v1 = ((double)((t1.time - t_initial.time) * 1000 + t1.millitm - t_initial.millitm)) / 1000;


		if (m_tipo==8) m_FactorAleatorio2=(double)get_random(1,150)/(double)200;
//		srand(m_iter_actual);
//		m_FactorAleatorio=(double)get_random(25,75)/(double)100;
		if (m_tipo_aleatoriedad==1) 
			m_FactorAleatorio=(double)get_random(40,90)/(double)100;
		...
		if (m_tipo_aleatoriedad==2) 
		{
			...
			{
				if (m_iter_actual%(m_max_iteracionesGrasp/5)==0)
				m_FactorAleatorio+=0.1;
			}
		}
		...
		if (m_tipo_aleatoriedad==5)
		{
			if (m_iter_actual%(500)==0 && m_iter_actual>0 && m_solbest!=0 && m_solbest!=peorSolucion)
			{
				Reactive=true;
				total=0;
				hastavalor=0;
...
*/	
		if (m_ValSolucion!=maxPosible && (m_ValSolucion<=(m_solbest+((double)0.25*(double)(peorSolucion-m_solbest)))) && m_HacerMejoraLocal==true)
		{	
		if (m_ReactiveMejora==true)
		{
			if (m_iter_actual%(m_max_iteracionesGrasp/10)==0 && m_iter_actual>0 && m_solbest!=0 && m_solbest!=peorSolucion)
			{
				ReactiveMejora=true;
				total=0;
				hastavalorM=0;
				...
				{	
					if (vec_vecesMejora[ix]!=0 && m_solbest!=0)
					{
						...
						//Problema de maximizar
						total+=pow((double)(peorSolucion-meanix)/(double)(-m_solbest+peorSolucion),3);
				...
				for (int ix=1;ix<5 ;ix++)
				{
					if (vec_vecesMejora[ix]!=0 && m_solbest!=0)
					{
						...
						double meanix=((double)vec_valorMejora[ix]/(double)vec_vecesMejora[ix]);
						double evalix=pow((double)(peorSolucion-meanix)/(double)(-m_solbest+peorSolucion),3);
...
//				m_tipo_mejora=get_random(1,4);
				...
				if (m_tipo_mejora==0)
					MejoraLocalIntercambioStrip();
				if (m_ValSolucion>peorSolucion)
					peorSolucion=m_ValSolucion;
				if (m_tipo_mejora<=4 && m_tipo_mejora>0)
				{
...
void CUTTING::MejoraLocalRapidaTabuStrip()
{
	...
	do
	{
	int num_bloques=(double)(0.75)*m_lista_bloques.size();
	int contador=0;
	itB=m_lista_bloques.begin();
	do{
		contador++;
...
//	Bloques_Hacia_Las_EsquinasStrip(m_lista_bloques,m_lista_perdidas,min);
	...
	{
		m_solbest=m_ValSolucion;
		m_ValSolucion=m_ValSolucion;
	}
}
//La idea es intercambiar una de las piezas por otra igual de ancho pero más estrecha
//Para ello al principio tengo una variable que me dice si esto puede ocurrir en un vector
//y para que anchos, ya que existirán anchos para los que no exista dos piezas de igual ancho
void CUTTING::MejoraLocalIntercambioStrip()
{
	std::list<BLOQUE>::iterator itBlo;
	std::vector<int> vec_suple;
	BLOQUE  B1(0,0,0,0);
	int contador=0,maxt=0;
	//tengo que tener que el tope de las piezas es una 
	for (itBlo=m_lista_bloques.begin();itBlo!=m_lista_bloques.end();itBlo++)
	{
		...
		if ((*itBlo).right<m_ValSolucion && (*itBlo).right>maxt)
			maxt=(*itBlo).right;
	}
	if (maxt>=m_solbest) return;
	if (contador>1) return;
	//Esta variable será uno si es el anterior
	//Por tenerlos ordenados por ancho
	//si existe alguno igual es el anterior o el siguiente
	if (m_Piezas[B1.GetNumero()].num!=m_Numpiezas-1 )
	{
		if (m_Piezas[B1.GetNumero()+1].ancho != m_Piezas[B1.GetNumero()].ancho )
		{
			return;
		...
		{
			int num=B1.GetNumero()+1;
			//Puede haber varios iguales
			bool continuar=false;
			do
			{
				continuar=false;
				if (m_Piezas[num].ancho == m_Piezas[B1.GetNumero()].ancho)	
				{
					vec_suple.push_back(num);
					continuar=true;
				}
				num++;
			}while(num!=(m_Numpiezas-1) && continuar==true);
	...
	//YA tengo en un vector los que son subceptibles de cambio
	//ahora se trata de recorrer el vector buscando cuales cambio
	//y cuanto se supone que voy a poder ganar
	//como estimación máxima de lo que voy a poder ganar es 
	std::vector<int>::reverse_iterator it;
//	std::list<BLOQUE> lista_suple;
	for (it=vec_suple.rbegin();it!=vec_suple.rend();it++)
	{
		//Busco un bloque cuanto se supone que voy a ganar
		if (m_ValSolucion-(m_Piezas[B1.GetNumero()].largo-m_Piezas[(*it)].largo)>=m_solbest)
			return;
		//Quito el bloque it de donde este y coloco B1
		//para ello solo tengo en cuenta los que no esten debajo de B1
		//Cambio B1 por el anterior 
		bool termina=false;
		for (itBlo=m_lista_bloques.begin();itBlo!=m_lista_bloques.end() && termina==false;itBlo++)
		{
			if ((*itBlo).GetNumero()==(*it))
			{
				if ((*itBlo).top>B1.bottom || (*itBlo).bottom<B1.top )
				{
					//Puede valer por lo que lo cambio 
					//y tengo que cambiar todos los siguientes
					std::list<BLOQUE> lista_suple(m_lista_bloques.begin(),m_lista_bloques.end());
					CambiarBloque((*itBlo),B1);
					int Valtemp=ContarSolucionStrip(m_lista_bloques);
					DibujarSolucion(m_lista_bloques,m_lista_perdidas,m_ValSolucion,0);

					if (Valtemp<m_solbest)
					{
						m_solbest=Valtemp;
						m_ValSolucion=Valtemp;
						printf("Puedo Cambiar Y mejora");
					}
					else
					{
						m_lista_bloques=lista_suple;
				...
				{
					if (m_Piezas[(*it)].max==1) termina=true;
				}
			}
		}

	}
	//Suponemos que tenemos un único bloque y que existe otro más estrecho
}
//Cambio el bloque 1 por el 2 y el 2 por el 1
void CUTTING::CambiarBloque(BLOQUE &B1,BLOQUE &B2)
{
	std::list<BLOQUE>::iterator itBlo;
	BLOQUE Btemp(B1);
	int max=0,alto=0,bajo=0;
	//Como estan ordenados por el más hacia la derecha
	for (itBlo=m_lista_bloques.begin();itBlo!=m_lista_bloques.end() ;itBlo++)
	{
		if (max==0)
		{
		if ((*itBlo).left==B1.left && (*itBlo).right==B1.right && (*itBlo).top==B1.top && (*itBlo).bottom==B1.bottom)
		{
			max=(*itBlo).right;
			(*itBlo).right=(*itBlo).left+B2.Width();
			(*itBlo).SetNumero(B2.GetNumero());
			bajo=(*itBlo).top;
			alto=(*itBlo).bottom;
		}
		}
		...
		{
			if (bajo<=(*itBlo).top && alto>(*itBlo).top)
			{
				(*itBlo).right=(*itBlo).right+(B2.Width()-Btemp.Width());
				(*itBlo).left=(*itBlo).left+(B2.Width()-Btemp.Width());
				if ((*itBlo).bottom>alto) alto=(*itBlo).bottom;
			}
			else
			{
				if (bajo<(*itBlo).bottom && alto<=(*itBlo).bottom)
				{
					(*itBlo).right=(*itBlo).right+(B2.Width()-Btemp.Width());
					(*itBlo).left=(*itBlo).left+(B2.Width()-Btemp.Width());
					if ((*itBlo).top<bajo) bajo=(*itBlo).top;
	...
	B2.right=B2.right-(B2.Width()-Btemp.Width());
	B2.SetNumero(Btemp.GetNumero());
...
//					EscribirSolucionStripLatex(m_lista_bloques);
			...
			if (m_PuedenRotarsePiezas==true && rect1.left+b1.Width()>(solmax+m_TotalAncho) && rect1.Height()!=m_W)
			{
				int kkaga=rect1.Height();
				int a1=0,a2=0;
				//anchura de los de abajo
				if (rect1.top==0) a1=m_L;
				else a1=Altura(rect1.top,0);
...
//		modo=2;
	...
	m_lista_perdidas.clear();
	//Al principio todo el tablero es perdida
	RECTAN PTotal(m_TrozoFijado,0,m_L,m_W);
	m_lista_perdidas.push_back(PTotal);
...
//	m_Dibujar=dibu;
	...
	if (m_Determinista!=true )
	{
		maxperdi=(((m_W*m_Cota)-m_total_superficie)/4)-m_PerdidaFijado;
//				maxperdi=get_random(0,maxperdi);
...
//			else
			...
			if (rect1.left+b1.Width()+m_MaxLargo>solmax )
			{
				...
				{
					estimacion=(double)get_random(100,200)/(double)100;
...
void CUTTING::Elijo_Menor_ListaStrip(RECTAN R, RECTAN &resultante, int tipo_orden,bool all)
{
	...
	for (std::list<RECTAN> ::iterator it=m_lista_perdidas.begin();it!=m_lista_perdidas.end() ;it++)
	{
		if (R.top==(*it).top && R.bottom==(*it).bottom && R.left==(*it).left && R.right==(*it).right)
			continue;
...
//	m_clientedc->SetBkMode(OPAQUE);
	...
	for (itB=m_lista_bloques_fijos.begin();itB!=m_lista_bloques_fijos.end();itB++)
	{
		m_clientedc->SelectObject(brushblanco);
		kk+=3;
		L=(*itB).Width();
...
//	}while(m_lista_perdidas.size()!=0 && itL1!=m_lista_perdidas.end() );
...
bool CUTTING::Existe_Perdida(std::list<RECTAN> &listaP,RECTAN &rect,int tipo)
{
	...
	for (std::list<RECTAN> ::iterator itL=listaP.begin(); (grande != true) && (itL!=listaP.end()) ;itL++)
	{
		RECTAN cuadra=(*itL);
			...
			&& ((*itL).bottom>=rect.bottom) && ((*itL).top<=rect.top))
		{
			...
			if (tipo==8)
			{
				...
				if (itL == listaP.end())
				{
					listaP.insert(listaP.end(), lista.begin(), lista.end());
					return true;
				}
			}
	...
	{
		listaP.insert(listaP.end(), lista.begin(), lista.end());
/*		for (std::list<RECTAN> ::iterator itL = lista.begin(); itL != lista.end(); itL++)
		{
...
//	if (tipo==3) tipo=get_random(1,2);
	...
	for (int i=0;i<m_Numpiezas;i++)
	{
		...
		for (int j=1;j<=(m_Piezas[i].max-m_Piezas[i].num) && basta!=true;j++)
		{
			...
			//Rotadas
			if (m_PuedenRotarsePiezas)
			{
				if (m_Piezas[i].ancho * j <= Rect.Width() && m_Piezas[i].largo <= Rect.Height())
				{
					if (((((val > max || (val == max && (((m_Piezas[i].ancho * j) * (m_Piezas[i].largo)) > (b1.bottom * b1.right)))) && (tipo == 1 || tipo == 3)) || ((val > max || (val == max && (((m_Piezas[i].largo * j) * (m_Piezas[i].ancho)) < (b1.bottom * b1.right)))) && (tipo == 2))) || (necesaria == true && necesaria2 == false)) || (tipo == 4 && val > max))
					{
						b1.bottom = m_Piezas[i].largo;
						b1.right = m_Piezas[i].ancho * j;
						b1.SetNumero(i);
						max = val;
						numero = j;
						//Si es necesaria meterla por las restricciones
			...
			if (m_Piezas[i].ancho<=Rect.Width() && m_Piezas[i].largo*j<=Rect.Height())
			{
				if ((((val>max || (val==max && (((m_Piezas[i].ancho)*(m_Piezas[i].largo*j))>(b1.bottom*b1.right)) )) && (tipo==1)) || ((val>max || (val==max && (((m_Piezas[i].largo)*(m_Piezas[i].ancho*j))<(b1.bottom*b1.right)) )) && tipo==2)) || (necesaria==true && necesaria2==false))
				{
					b1.bottom=m_Piezas[i].largo*j;
					b1.right=m_Piezas[i].ancho;
					b1.SetNumero(i);
...
int CUTTING::CotaInferiorL1()
{
	std::sort(m_Piezas.begin(),m_Piezas.end(),orden_stripAltura());

	int k=0;
	int max=0,sum1=0,sum2=0,maxThis=0;
	for (register int ik=1;ik<m_Numpiezas ;ik++)
	{
			sum1+=m_Piezas[ik].ancho;
			if (sum1>m_W) 
			{
				sum1-=m_Piezas[ik].ancho;
				break;
	...
	for (int ik=(k+1);ik<m_Numpiezas;ik++)
	{
		int il=0;
		if (m_Piezas[ik].ancho+sum1>m_W)
		{
			sum2=0;
			for (register int ij=0;ij<ik ;ij++)
			{
				
				sum2+=m_Piezas[ij].ancho;
				if (sum2+m_Piezas[ik].ancho>m_W)
				{
					il=ij;
					break;
				}
			}
			maxThis=m_Piezas[il].largo+m_Piezas[ik].largo;
...
int CUTTING::CotaInferiorL2()
{
	int max=0;
	for (register int alpha=1; alpha<=m_W/2;alpha++)
	{
		int sum1=0,sum21=0,sum22=0;
		for (register int ik=0;ik<m_Numpiezas;ik++)
		{
			
			//J1
			if (m_Piezas[ik].ancho>m_W-alpha)
				sum1+=m_Piezas[ik].largo;
			//J2
			if (m_Piezas[ik].ancho<=m_W-alpha && m_Piezas[ik].ancho>m_W/2)
			{
				sum1+=m_Piezas[ik].largo;
				sum22+=((m_W-m_Piezas[ik].ancho)*m_Piezas[ik].largo);
			}
			//j3
			if (m_Piezas[ik].ancho<=m_W/2 && m_Piezas[ik].ancho>=alpha)
			{
				sum21+=m_Piezas[ik].largo*m_Piezas[ik].ancho;
			}
		}
		int maxThisalpha=sum1+__max(0,ceil((double)(sum21-sum22)/(double)m_W));
		if (maxThisalpha>max)
			max=maxThisalpha;
	}
	return max;
...
void CUTTING::WriteSolution()
{
	...
	if (m_lista_bloques.size() != 0)
	{
		...
		{
			int L = m_Piezas[(*it).GetNumero()].largo;
			int w = m_Piezas[(*it).GetNumero()].ancho;
			bool rotada = false;
			if (m_PuedenRotarsePiezas == true)
			{
				if (!((*it).Width() % L == 0 && (*it).Height() % w == 0))

				{
					w = m_Piezas[(*it).GetNumero()].largo;
					L = m_Piezas[(*it).GetNumero()].ancho;
					rotada = true;
				}
			}
			int x = (*it).left;
			...
			for (register int ix = 1; ix <= ((*it).Width() / L); ix++)
			{
				if (ix == 1) x = (*it).left;
				for (register int iy = 1; iy <= ((*it).Height() / w); iy++)
				{
					if (iy == 1) y = (*it).top;
					fprintf(fin3, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", rotada, (*it).GetNumero(), (*it).GetNumero(), x, y, (x + L), y + w, 1,m_Piezas[(*it).GetNumero()].valor);
					y = y + w;
				}
				x = x + L;
			}
