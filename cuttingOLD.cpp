...
void CUTTING::LeerProblema(char *name)
{
	...
	int Largop,Anchop,minp,maxp,valp,kk;
	fin2=fopen(name,"r+");
	if (fin2==NULL)
	{
...
//						int kk2=m_Piezas[i].largo;
	...
	{
		b1.right=m_Piezas[lista_candidatos[elegido].second.nump].ancho*lista_candidatos[elegido].second.numero;
		b1.bottom=m_Piezas[lista_candidatos[elegido].second.nump].largo;
	}
...
*/	//Tengo una solución inicial
	...
	int cantidad_minima=0.25*(m_L*m_W)/(m_Numpiezas);
...
//			m_lista_tabu_aumen.pop_front();
			if (m_iter_actual==494 && m_lista_tabu_new.size()==1)
				int kk=9;
...
//		printf("Iter %d\n",m_iter_actual);
		...
		{
//			printf(" Mejora %d\t%d\t%d\n",m_iter_actual,kkk-m_ValSolucion,m_ValSolucion);
//			EscribirSolucionStripLatex(m_lista_bloques);
		}
//		if (kkk-m_ValSolucion<0)
...
//			printf("SOLBEST\t%d\t%d\n",m_solbest,m_iter_actual);
	...
	if (posi==true) m_Posible=true;
	printf("Numero de Piezas %d  W %d %d  Mejor Solucion %d",m_Numpiezas,m_W,m_iter_actual,m_solbest);
...
void CUTTING::MejoraLocalRapidaStrip()
{
	...
	do
	{
	int num_bloques=(double)(0.4)*m_lista_bloques.size();
	int contador=0,contador2=0;
	int kk=m_lista_bloques.size();
	itB=m_lista_bloques.begin();
	do{
		contador++;
		if (contador>num_bloques)
...
//	Bloques_Hacia_Las_EsquinasStrip(m_lista_bloques,m_lista_perdidas,min);
	...
	m_tipo=2;
	DibujarSolucion(m_lista_bloques,m_lista_perdidas,m_ValSolucion,0);
	RellenarStrip(m_lista_bloques,m_lista_perdidas,99999);
	m_tipo=guartipo;
...
//	printf("quito %d de %d\n",contador2,kk);
	...
	int sol1=ContarSolucion(m_lista_bloques);
	if (m_solbest==34)
		int perrazo=9;
	int mLguardo=m_L;
...
//	Bloques_Hacia_Las_EsquinasStrip(m_lista_bloques,m_lista_perdidas,0);
	...
	JuntarPerdida(m_lista_perdidas,'H');
	DibujarSolucion(m_lista_bloques,m_lista_perdidas,m_ValSolucion,0);
	int maxBloqueAncho=0,bloque_seleccionado,cuans;
...
void CUTTING::RellenarStrip(std::list<BLOQUE> &listaB,std::list<RECTAN> &lista,int con_este_numero_no)
{
	/*
	//Tengo que rellenar otra vez ese rectángulo pero con piezas diferentes
	//de la que he quitado 
...
//				int quedan_gran=__min(m_Piezas[pieza].max-m_Piezas[pieza].num,rect1.Height()/m_Piezas[pieza].ancho);
				...
				if (rect1.left+pieza_largo>solmax)
				{
					...
					else
					{
						if ((rect1.Height()-b1.Height())<pieza_ancho*quedan_gran)//piezas pocas
						{
							...
							{
								int areaquedasig=AreaRestanteSiguiente(rect1.left,b1.Width(),pieza_ancho*quedan_gran,pieza_largo);
				...
				else
				{
					if ((rect1.Height()-b1.Height())<pieza_ancho*quedan_gran)//piezas pocas
					{
						if (rect1.left+b1.Width()+pieza_largo>solmax)
						{
							int areaquedasig=AreaRestanteSiguiente(rect1.left,b1.Width(),m_Piezas[pieza].ancho*quedan_gran,pieza_largo);
							if ((areaquedasig)>=estimacion*m_AreaTotalRestante)
							{
								meto_larga=true;
							}
	...
	//				maxperdi=get_random(0,maxperdi);
				...
				if (rect1.left+m_Piezas[pieza].largo>maxsol)
				{
					...
					else
					{
						...
						{
							int areaquedasig=AreaRestanteSiguiente(rect1.left,b1.Width(),m_Piezas[pieza].ancho*quedan_gran,m_Piezas[pieza].largo);
//							int areaquedasig=AreaRestanteSiguiente(rect1,b1.Width(),b1.Height(),m_Piezas[pieza].ancho*quedan_gran,m_Piezas[pieza].largo);
							if ((areaquedasig)>=estimacion*(m_AreaTotalRestante+maxperdi))
							{
								meto_larga=true;
...
//	else a1=Altura(Rect.top,0);
	...
	if (altura_abajo>=Rect.left+B.Width()) return 0;
	if (Rect.bottom==m_W) return 1;
	//anchura de los de arriba
	int altura_arriba=0;
//	if ((m_W-Rect.bottom)<m_MinAncho) return 1;
...
int CUTTING::AreaRestanteSiguiente(int izquierda,int largo_suple,int ancho_pieza, int long_pieza)
{
...
//cambiar		if ( (*itB).Height()>=ancho_pieza)
		if ((*itB).left>izquierda && (*itB).Height()>=ancho_pieza)
		{
			max+=(*itB).left;
		}
	}
	...
	if (max==0) 
	{
		max=izquierda+largo_suple;
	}
	else
	{
		max=__min(max,izquierda+largo_suple);
	}
	//¿EStaría mal tendría que poner si se pasa de vueltas?
	if (m_Determinista!=true && max>m_solbest) return 2*m_AreaTotalRestante;
	return AreaRestante(max+long_pieza);
