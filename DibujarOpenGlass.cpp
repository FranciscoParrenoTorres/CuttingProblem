...
FILE *fin2;
	...
	if (fin2==NULL)
	{
		printf("Problemas al leer el archivo %d, no se encuentra el archivo temp2.txt" );
		exit(4);
	}
	int H=0;
	int W=0;
...
//	factor=0.1;
	...
	izquierda = -48 ;
	glColor3f(0, 0, 0);
	glRasterPos2d(izquierda + 0*factor, H*1.05 * factor);
	texto("Cada pieza indica: ancho,largo id,val rotada", 2);
	int val = 0;
	while (!feof(fin2))
//	for (int k=0;k<piezas;k++)
	{
		fscanf(fin2,"%d %d %d %d %d %d %d %d %d",&pieza,&stack,&seq,&x,&y, &X,&Y,&plate,&val);
		...
		if (plate2 != plate)
		{
			...
			glColor3f(0, 0, 0);
			glRasterPos2d(izquierda + 0 * factor, -H*0.1 * factor);
			
			string kk = to_string(plate);
//			string 
			texto(nombre, 3);
			texto(" Medidas: ",3);
			kk = to_string(W);
			...
			kk = to_string(rotate);
			texto(kk, 3);
			texto(" Obligatorias: ", 3);
			kk = to_string(restringido);
			texto(kk, 3);
//			texto(obj, 3);
		}
		izquierda = -48 + (((W + 100)*factor)*(plate - 1));
...
//		texto(info_piece);
		...
		texto(stack, val);
		texto(pieza, -1);
//		glRasterPos2d(izquierda + x*factor, y*factor + (((Y - y) / 2)*0.9)*factor);
...
void texto(string X,int size)
{
	...
	for (i = 0; i < len; i++)
	{
		if (size==1) glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_10   , X[i]);
			if (size==2) glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18    , X[i]);
			if (size==3) glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24    , X[i]);
 ...
 int DibujarOpenGL( )
{
	...
	// glutInitWindowSize(1600, 800);
	 ...
	 strcat(intStr, " Cutting visualization");

	 int  Win2 = glutCreateWindow(" Cutting visualization");
//	 glutFullScreen();
