...
void main(int argc,char **argv)
{
	...
	int Seconds=100;
	int visible = 0;
	int tipo_problema = 1;//0 strip 1 corte
	int rotation = 0; //No se pueden rotar
	...
	{               //Se comprueba que haya pares de argumentos (y al menos un par)
		while (argc > 1)
		{
			...
			{
				sscanf(argv[2], "%d",
					&rotation); //Type=0 is the usual //Type=1 is the new one
			}
			else if (strcmp(argv[1], "-Problema") == 0)
			{
				sscanf(argv[2], "%d",
					&tipo_problema); //Type=0 is the usual //Type=1 is the new one
			}
			argc -= 2;
			argv += 2;
		}
...
//	tipo_mejora=0;
	...
	Problema.SetPuedenRotarsePiezasGrasp(rotation);
	...
	if (tipo_problema == 1)
	{
		Problema.SetDibujar(false);
		Problema.SetEmpiezaSolGrasp(false);
		sol = Problema.Tabu(false);
		Problema.SetDibujar(visible);
